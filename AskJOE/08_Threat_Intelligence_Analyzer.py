...
# Optional Ghidra helper for consistent config location
try:
    from ghidra.app.script import GhidraScriptUtil
    _HAVE_GHIDRA_SCRIPT_UTIL = True
except ImportError:
    _HAVE_GHIDRA_SCRIPT_UTIL = False

# Setup standardized logging
logger, log_file = setup_logging("threat_intelligence")

# Use println from execution context: when run from AskJOE launcher it goes to the AskJOE panel;
# when run standalone from Ghidra it goes to the script console. Do not define println here.
...
class ThreatIntelligenceAnalyzer:
    ...
    def load_config(self):
        ...
        try:
            ...
            # Prefer the same USER_SCRIPTS_DIR-based path used by other AskJOE modules
            if _HAVE_GHIDRA_SCRIPT_UTIL:
                try:
                    ghidra_script = GhidraScriptUtil()
                    usr_dir = ghidra_script.USER_SCRIPTS_DIR
                    possible_paths.append("{}/AskJOE/config.ini".format(usr_dir))
                except Exception as e:
                    log_warning(logger, "Could not resolve USER_SCRIPTS_DIR via GhidraScriptUtil: {}".format(e))

            # Fallback locations (script in AskJOE/, config in same dir)
            ...
            if not config_path:
                log_warning(
                    logger,
                    "Config file not found in any of these locations: {}".format(
                        possible_paths
                    ),
                )
                log_warning(logger, "Please create AskJOE/config.ini with your API keys")
                # Fallback config_dir to AskJOE folder
                self.config_dir = os.path.dirname(os.path.abspath(__file__))
                return {}
            
            # Remember where the config was found so we can place logs consistently
            ...
            # Fallback config_dir to AskJOE folder
            self.config_dir = _ASKJOE_DIR
            return {}
    
    def export_results(self, results, file_hash):
        """Export threat intelligence results to a JSON file for external tooling."""
        try:
            # Resolve base directory near the config file when possible
            base_dir = getattr(self, "config_dir", _ASKJOE_DIR)
            log_dir = (
                self.config.get("LOGGING", {}).get("log_directory", "logs")
                if isinstance(self.config, dict)
                else "logs"
            )
            if not os.path.isabs(log_dir):
                # When base_dir is AskJOE folder, config "AskJOE/logs" -> "logs"
                if log_dir.startswith("AskJOE/"):
                    log_dir = log_dir[7:]
                log_dir = os.path.join(base_dir, log_dir)
            
            safe_hash = file_hash or "UNKNOWN"
            json_path = os.path.join(
                log_dir, "threat_intel_{}.json".format(safe_hash)
            )
            
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(results, f, indent=2, default=str)
            
            log_info(logger, "Threat intelligence results exported to {}".format(json_path))
        
        except Exception as e:
            log_warning(logger, "Could not export threat intelligence results: {}".format(e))
...
def main():
    ...
    try:
        ...
        # Normalize Windows-style paths that may be prefixed with a leading slash (e.g. "/C:/path")
        try:
            if os.name == "nt" and isinstance(program_path, str):
                if program_path.startswith("/") and len(program_path) > 2 and program_path[2] == ":":
        ...
        if results and "error" not in results:
            ...
            # Export structured JSON results for external tooling
            try:
                analyzer.export_results(results, file_hash)
            except Exception as export_error:
                log_warning(logger, "Failed to export threat intelligence results: {}".format(export_error))
