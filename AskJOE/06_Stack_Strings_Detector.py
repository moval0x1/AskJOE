...
def load_config():
    ...
    try:
        ...
        # Prefer the same USER_SCRIPTS_DIR-based location used elsewhere in AskJOE
        try:
            ...
            log_debug(logger, "Could not resolve USER_SCRIPTS_DIR via GhidraScriptUtil: {}".format(e))

        # Fallback to script-relative path (AskJOE folder)
        if not config_path:
...
def calculate_string_entropy(s):
    ...
    try:
        ...
        log_debug(logger, "Error calculating string entropy for '{}': {}".format(s, e))
        return 0.0
...
def detect_stack_strings(operation):
    ...
    try:
        ...
        println("[+] Config: min_length={}, max_per_type={}".format(min_string_length, max_strings_per_type))
        ...
        while instructions.hasNext():
            ...
            if mnemonic in ['PUSH', 'MOV']:
                # Check operand 1 first (e.g. MOV reg, imm), then operand 0 (e.g. PUSH imm)
                try:
                    scalar_value = instruction.getScalar(1)
                    if scalar_value is None and instruction.getNumOperands() >= 1:
                        scalar_value = instruction.getScalar(0)
        ...
        if stack_strings:
            ...
            println("")
            println("### Statistics")
            println("- **Candidates scanned:** {}".format(total_candidates))
            println("- **Filtered out:** {} ({:.1f}%)".format(
                filtered_out, (filtered_out / total_candidates * 100) if total_candidates > 0 else 0))
            println("- **Valid strings:** {}".format(len(stack_strings)))
            println("")
            ...
            for string_type in sorted(by_type.keys()):
                println("### {} Strings".format(string_type))
                ...
                println("")
            println("### Summary")
            println("- **Total:** {} stack strings".format(len(stack_strings)))
            ...
            println("## Stack Strings – No valid strings")
            println("")
